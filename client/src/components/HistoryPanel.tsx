...
export function HistoryPanel() {
  ...
  const loadHistory = () => { setLoading(true); try { setApps(getLocalHistory()); } finally { setLoading(false); } };
  useEffect(() => { loadHistory(); }, []);

  const handleDelete = async (id: string) => {
    setDeleting(id);
    try { deleteFromHistory(id); deleteApp(id).catch(() => {}); setApps((prev) => prev.filter((a) => a.id !== id)); } finally { setDeleting(null); }
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <AnimatePresence>
        {apps.map((app) => (
          <motion.div key={app.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }}
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-medium text-[var(--text-primary)] truncate">{app.info?.name || app.originalName}</span>
                  {app.status === 'signed' ? <CheckCircle className="w-4 h-4 text-green-400 flex-shrink-0" /> : app.status === 'error' ? <XCircle className="w-4 h-4 text-red-400 flex-shrink-0" /> : null}
                </div>
                <div className="text-xs text-[var(--text-muted)] mt-0.5">
                  {app.info?.bundleId} &middot; v{app.info?.version}{app.info?.size ? ` · ${formatFileSize(app.info.size)}` : ''}
                </div>
                {app.signedAt && <div className="text-xs text-[var(--text-muted)] mt-1">{t('signed')} {new Date(app.signedAt).toLocaleString()}</div>}
                {app.error && <div className="text-xs text-red-400 mt-1">{app.error}</div>}
              </div>
